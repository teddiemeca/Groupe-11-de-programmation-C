...
void calcul_moyenne_definitive(etudiant*liste, int nb_etudiant)
{
    for (etudiant* cpt = liste ; cpt < (liste + nb_etudiant) ; cpt++ )
    {
        if (cpt->moyenne_brute < 10){
            cpt->bonus = 1;
        }
        else if (cpt->moyenne_brute <= 15){
            cpt->bonus = 0.5;
        }
        else{
            cpt->bonus = 0.0;
        }
        cpt->moyenne_definitive = cpt->moyenne_brute + cpt->bonus;
    }
}
