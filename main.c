...
void calculer_moyennes(etudiant liste[], int nb_etudiants)
{
    for (int i = 0; i < nb_etudiants; i++) {
        liste[i].moyenne_brute = 0.45 * liste[i].note_dst + 0.55 * liste[i].note_examen;
    }
}


void calcul_moyenne_definitive(etudiant liste[], int nb_etudiants)
{
    for (int i = 0; i < nb_etudiants; i++) {
        if (liste[i].moyenne_brute < 10) {
            liste[i].bonus = 1;
        } else if (liste[i].moyenne_brute <= 15) {
            liste[i].bonus = 0.5;
        } else {
            liste[i].bonus = 0;
        }
        liste[i].moyenne_definitive = liste[i].moyenne_brute + liste[i].bonus;
    }
}
